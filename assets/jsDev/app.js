...
**************************************************/	
	...
	function TabPanel(o) {
		...
		_.handleUserEvent = function (e) {
			...
			if (e.type === 'keydown') {
				...
				if (t.hasAttribute('role') && t.getAttribute('role', 'tab') && e.ctrlKey === false) {
					...
					if (kc === 37 || kc === 38 || kc === 33) {
						if (t.previousSibling) t.previousSibling.focus();
						else n.lastChild.focus();
					}
					//right or down or page down
					if (kc === 39 || kc === 40 || kc === 34) {
						if (t.nextSibling) t.nextSibling.focus();
						else n.firstChild.focus();
					}
			...
			if ((e.type === 'focus' || e.type === 'click') && t.hasAttribute('aria-controls') && t.getAttribute('aria-selected') !== 'true') {
				t.removeAttribute('tabindex');
				_.displayTab(t);
				e.preventDefault();
				e.stopPropagation();
			}
			if (e.type === 'click' && (t.hasAttribute('role') && (t.getAttribute('role') === 'tab') || t.hasAttribute('data-delPanel'))){
			 	e.preventDefault();
			 }
			e.stopPropagation();
			//return false;
		...
		_.displayTab = function (t) {
			...
			for (i = 0; i < tp.length; ++i) {
				...
				if (tp[i].getAttribute('aria-selected') === 'true') {
					...
					if(tp[i].classList.contains('isAnimated')) tp[i].classList.remove('isAnimated');
			...
			if (t.hasAttribute('aria-selected') && t.getAttribute('aria-selected') !== true) {
				...
				ap.classList.remove('isHidden');
				if (_.o.secureTabIndex) {
					_.secureTabIndex(lp, true);  
					_.secureTabIndex(ap, false);
				}
				t.setAttribute('aria-selected', true);
		...
		_.generateTabs = function (w) {
			...
			for (var it = 0; it < s.length; it++) {
				...
				id = s[it].id;
				if (h !== 0 && s[it].querySelector(h)){ th.push(id);}
...
*/
	function pageLoaded() {
		var list = document.querySelectorAll('.tbz-tabs'),
		    options = {
		    	anim: true,
		    },
