...
function TabPanel(o) {
	...
	_.handleUserEvent = function (e) {
		...
		if (e.type === 'keydown') {
			...
			if (this.hasAttribute('role') && this.getAttribute('role', 'tablist')) {
			...
			if (e.ctrlKey === true) {
				...
				if (kc == 38 || kc == 40) at.focus();
				//ctrl + left : prev tab
				if (kc == 37) {
					at.previousSibling.focus();
				}
				//ctrl + right : next tab
				if (kc == 39) {
					at.nextSibling.focus();
				}
...
/*
 	Exemple d'initialisation du script
 	========================
